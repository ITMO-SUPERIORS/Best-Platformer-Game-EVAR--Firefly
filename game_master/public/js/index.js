...
const MainMenu = new Phaser.Class ({
  ...
  create: function() {
    ...
    this.add.text(320, 250, "УПРАВЛЕНИЕ СТРЕЛКАМИ \nИЛИ КНОПКАМИ WASD НА КЛАВИАТУРЕ", style).setScrollFactor(0);
...
const PlatformerScene = new Phaser.Class ({
  Extends: Phaser.Scene,
  ...
  preload: function() {
    ...
    this.load.image("pike", "./assets/images/pike.png");
  ...
  create: function() {
    ...
    this.groundLayer.forEachTile(tile => {
      if (tile.index === this.spike_arr.get('spikes')) {
        const spike = this.spikeGroup.create(tile.getCenterX(), tile.getCenterY(), "pike");
  ...
  moveCoin: function() {
    ...
    const itemsX = Array(Phaser.Math.Between(this.playerPosX + 200, this.playerPosX + 450), Phaser.Math.Between(this.playerPosX - 200, this.playerPosX - 450));
    this.coin.x = itemsX[Math.floor(Math.random()*itemsX.length)];
    
    this.playerPosY = this.player.sprite.y;
    const itemsY = Array(Phaser.Math.Between(this.playerPosY + 128, this.playerPosY + 256), Phaser.Math.Between(this.playerPosY - 128, this.playerPosY - 360));
